...
#Agent/Security_packet instproc recv {from rtt mess originmess} {
#	$self instvar node_
#	puts "node [$node_ id] received packet from \
#              $from with trip-time $rtt ms - contend: $mess - decrypted $originmess"
#}
...
for {set i 1} {$i < 2} {incr i} {
	set result [expr $i /2]
	$ns at $result "$p0 send fat"
#	$ns at [expr $result + 0.01] "$p0 send Rathan" 
#	$ns at [expr $result + 0.02] "$p0 send  Ajey"
